...
public interface ProjectRepository extends JpaRepository<Project, String>, Searchable {
  ...
  @Query("SELECT COUNT(p) FROM Project p WHERE p.tenant.tenantId = :tenantId AND p.projectStatus = :status")
  long countByTenantTenantIdAndProjectStatus(@Param("tenantId") String tenantId,
      @Param("status") com.shegami.hr_saas.modules.mission.enums.ProjectStatus status);
